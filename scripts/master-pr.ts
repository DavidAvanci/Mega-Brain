import { nextDeploySlot } from '../src/deployWindow.ts'
import { countCommits, currentBranch, defaultBranch, existingPrUrl, featureBranch, gh, git, hasRef } from './lib/git.ts'
...
function prHasConflicts(cwd: string, url: string): boolean {
  try {
    const state = gh(cwd, 'pr', 'view', url, '--json', 'mergeable,mergeStateStatus', '--jq', '[.mergeable, .mergeStateStatus] | join(" ")')
...
async function masterPr(repo: { name: string; path: string }, task: ReturnType<typeof taskInfo>, body: string): Promise<string> {
  const cwd = repo.path
  ...
  if (prHasConflicts(cwd, url)) throw new Error(`${repo.name}: o PR para ${base} continua com conflitos após a resolução automática`)
  return url
}
...
async function main() {
  ...
  finish(
    ok,
    [`PRs: ${Object.values(prs).join(' ')}`, slot.open ? 'Dentro da janela de deploy' : `Deploy previsto: ${slot.label} (BRT)`, ...errors].join('\n'),
  )
  process.exit(ok ? 0 : 1)
}

...
if (!windows) {
  throw new Error('O shell do Mega Brain deve ser iniciado no Windows. Execute npm run tauri:dev pelo PowerShell no checkout Windows.')
}
...
const backendBuild = resolve(repositoryRoot, 'scripts/server-build.mjs')
...
function localDevelopmentEnvironment() {
  ...
  for (const key of ['NO_PROXY', 'no_proxy']) {
    const entries = (environment[key] ?? '').split(',').map((entry) => entry.trim()).filter(Boolean)

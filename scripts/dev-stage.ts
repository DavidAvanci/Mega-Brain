...
async function repairFailedTests(task: ReturnType<typeof taskInfo>): Promise<ItemOutcome[]> {
  ...
  for (const item of failed) {
    ...
    if (!item.repo || !existsSync(repoPath(item.repo))) {
      outcomes.push({ status: 'blocked', note: `${item.id}: Repo desconhecido: ${item.repo || '(sem heading ## repo)'}` })
      continue
    }
    try {
      ensureWorktree(wsPath, item.repo, task.id, task.branch)
    } catch (error) {
      outcomes.push({ status: 'blocked', note: `${item.id}: Worktree indisponível: ${error instanceof Error ? error.message : error}` })
      continue
    }
    ...
    try {
      ...
      outcomes.push({ status: 'failed', note: `${item.id}: Falha ao criar a worktree: ${error instanceof Error ? error.message : error}` })
      continue
    }

...
async function measureOnce() {
  ...
  try {
    ...
    throw new Error(
      `Amostra de readiness falhou: ${error instanceof Error ? error.message : 'erro desconhecido'}${stderr ? ' (stderr capturado e omitido)' : ''}`,
    )
  } finally {
    await stop(child)
  }
  ...
  return new Promise((resolve, reject) => {
    ...
    const onData = (chunk) => {
      ...
      try {
        ...
        if (value?.type !== 'mega-brain-ready' || !Number.isInteger(value.port) || value.port < 1 || value.port > 65535)
          throw new Error('ready inválida')
        finish(undefined, value)
      } catch {
        finish(new Error('ready inválida'))
      }
    }
...
function parseRuns(value) {
  ...
  if (!Number.isInteger(parsed) || parsed < 1 || parsed > 100)
    throw new Error('MEGA_BRAIN_BENCHMARK_RUNS deve ser inteiro entre 1 e 100.')
  return parsed
}
function parseTimeout(value) {
  const parsed = Number(value ?? 10_000)
  if (!Number.isInteger(parsed) || parsed < 100 || parsed > 60_000)
    throw new Error('MEGA_BRAIN_BENCHMARK_TIMEOUT_MS deve ser inteiro entre 100 e 60000.')
  return parsed
}

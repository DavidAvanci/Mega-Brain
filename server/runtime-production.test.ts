...
const request = (method: string, path: string, body?: unknown) => ({ method, path, body, query: new URLSearchParams(), headers: {} })
...
describe('production runtime composition', () => {
  test('registers every domain route without I/O or child processes at startup', async () => {
    ...
    const child = Object.assign(new EventEmitter(), {
      kill: () => true, unref: () => undefined, pid: 12345, stdout: new EventEmitter(), stderr: new EventEmitter(),
    })
    const runner: ProcessRunner = {
      spawn: () => { spawns++; return child as any },
      execFileSync: () => '',
      execFile: (_command, _args, _options, callback) => callback(null, '', ''),
    }
    const config = loadMegaBrainConfig({ env: { WORKSPACE_DIR: workspace, MEGA_BRAIN_CLAUDE_HOME: claude, MEGA_BRAIN_POWERSHELL_BIN: '/bin/sh' }, homeDir: base })
    ...
    expect((await runtime.handle({ ...request('GET', '/api/chat'), query: new URLSearchParams('name=card') }))?.status).toBe(200)

...
describe('standalone HTTP limits', () => {
  ...
  afterEach(async () => { await Promise.all(servers.map((server) => server.stop())); servers.length = 0 })

  const start = async (runtime: ServerRuntime, limits = {}) => {
    const server = createStandaloneServer({ config: loadMegaBrainConfig({ env: {}, homeDir: '/tmp/mega-brain-http-limits-test' }), runtime, listen: { port: 0 }, sessionToken: token, limits })
    servers.push(server); await server.start(); return `http://127.0.0.1:${server.address().port}`
  }

  test('accepts the exact body limit and rejects streamed excess with a stable 413', async () => {
    const base = await start({ register() {}, handle: async (input) => ({ status: 200, body: input.body }) }, { maxBodyBytes: 7 })
  ...
  test('rejects misleading Content-Length framing before it reaches domain code', async () => {
    ...
    const response = await rawRequest(base, '/short', { authorization: `Bearer ${token}`, 'content-length': '1' }, '"123456"')
  ...
  test('times out short work but leaves long chat/SSE work running', async () => {
    ...
    const base = await start({ register() {}, handle: async (input) => input.path === '/api/chat/send' ? await never() : await never() }, { shortRouteTimeoutMs: 15 })
...
function rawRequest(base: string, path: string, headers: Record<string, string>, body: string): Promise<{ status: number; body: string }> {
  const url = new URL(path, base)
  return new Promise((resolve, reject) => {
    const req = request({ host: url.hostname, port: url.port, path, method: 'POST', headers }, (response) => {
      let result = ''; response.setEncoding('utf8'); response.on('data', (chunk) => { result += chunk }); response.on('end', () => resolve({ status: response.statusCode ?? 0, body: result }))
    })

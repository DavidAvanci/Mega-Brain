...
const stage = { name: 'run-task-checklist', script: 'scripts/commands/dev-stage.ts' }

describe('stage script command', () => {
  test('reuses the self-contained desktop runtime instead of requiring tsx', () => {
    expect(
      stageScriptCommand('/workspace/card', stage, {
  ...
  test('keeps the source script path outside the bundled desktop runtime', () => {
    expect(
      ...
      [join('/source', 'scripts/commands/dev-stage.ts'), '/workspace/card'],
    ])
  })
})

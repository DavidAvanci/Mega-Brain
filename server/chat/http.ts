...
export const chatHistoryHttp = (service: ChatService): ApiHandler => legacyJsonHandler(async (r) => ({
  status: 200,
  headers: JSON_HEADERS,
  body: await service.history(String(r.query.get('name') ?? '')),
}))
export const chatAbortHttp = (service: ChatService): ApiHandler => legacyJsonHandler(async (r) => ({
  status: 200,
  headers: JSON_HEADERS,
  body: { ok: service.abort(String((r.body as any)?.name ?? '')) },
}))

export const chatSendHttp = (service: ChatService): SseHandler<ChatEvent> => async (r) => {
  const body = r.body as { name?: unknown; text?: unknown } | undefined
  return {
    status: 200,
    headers: SSE_HEADERS,
    cancel: () => { service.abort(String(body?.name ?? '')) },
    stream: (emit) => {
      try {

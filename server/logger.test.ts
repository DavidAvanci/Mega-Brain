...
describe('structured backend logging', () => {
  test('redacts nested secrets, errors, URLs and headers before the sink receives JSONL', () => {
    ...
    logger.event('test.failure', {
      token: canaries[0],
      jiraEmail: canaries[1],
      jiraToken: canaries[2],
      prompt: canaries[3],
      fileContent: canaries[4],
      headers: { authorization: `Bearer ${canaries[0]}` },
      url: `http://user:${canaries[2]}@localhost/x?prompt=${canaries[3]}`,
      nested: { error: nested, content: canaries[4] },
      error: nested,
    })
  ...
  test('keeps crash-like diagnostic objects metadata-only, including Error causes and codes', () => {
    ...
    const error = Object.assign(
      new Error(`${canaries[0]} ${canaries[1]}`, {
  ...
  test('logs request correlation and failures without query, auth header, or nested error text', async () => {
    ...
    const runtime: ServerRuntime = {
      register: () => undefined,
      handle: async () => {
        throw new Error(`${prompt} ${fileContent}`, { cause: new Error(jiraEmail) })
      },
    }
    const server: StandaloneServer = createStandaloneServer({
      config: loadMegaBrainConfig({ env: {}, homeDir: '/tmp/mega-brain-log-test' }),
      runtime,
      listen: { port: 0 },
      sessionToken: token,
      logger,
    ...
    try {
      ...
      const response = await fetch(
        `http://127.0.0.1:${server.address().port}/api/workspace?prompt=${encodeURIComponent(prompt)}`,
        {

...
describe('allowed transport differences', () => {
  test('only standalone applies bearer auth, closed CORS, and a boundary-specific malformed JSON status', async () => {
    ...
    const cors = await fetch(`${node.base}/api/jira/ready`, { headers: headers(node, { Origin: 'http://tauri.localhost' }) })
    expect(cors.headers.get('access-control-allow-origin')).toBe('http://tauri.localhost')
    const invalidOrigin = await fetch(`${node.base}/api/jira/ready`, { headers: headers(node, { Origin: 'http://evil.example' }) })
    expect(invalidOrigin.status).toBe(403)
    for (const transport of [node, vite]) {
      const malformed = await fetch(`${transport.base}/api/jira/ready`, { method: 'POST', headers: headers(transport, { 'Content-Type': 'application/json' }), body: '{' })
...
async function start(name: Transport['name']): Promise<Transport> {
  ...
  if (name === 'node') {
    ...
    for (const [key, handler] of fixture.routes) { const [method, path] = key.split(' ', 2); runtime.register(method, path, handler) }
    const server = createStandaloneServer({ config: loadMegaBrainConfig({ env: {}, homeDir: '/tmp/mega-brain-transport-parity' }), runtime, listen: { port: 0 }, sessionToken: token })
    standalone.push(server); await server.start()
  ...
  harness.push(server); await listen(server)
  return { name, base: `http://127.0.0.1:${(server.address() as AddressInfo).port}` }
}

function connectMounted(adapter: ReturnType<typeof createViteApiAdapter>, req: IncomingMessage, res: ServerResponse): void {
  const url = req.url ?? '/'
...
function fakeRoutes(): { routes: ReadonlyMap<string, Route>; readonly cancelled: number } {
  ...
  let cancelled = 0
  const workspace = workspaceHttp({ handle: async (path, method, query) => { if (query.get('name') === 'invalid') throw new Error('falha simulada'); return { route: `${method} ${path}` } } })
  const absoluteWorkspace: ApiHandler = (request) => workspace({ ...request, path: request.path.slice('/api/workspace'.length) || '/' })
  const chat = {
    history: async (card: string) => { if (!card || card === 'invalid') throw new Error('falha simulada'); return { sessionId: null, entries: [] } },
    abort: () => true,
    send: (_card: string, _text: string, emit: (event: ChatEvent) => void) => { emit({ type: 'text', text: 'primeiro' }) },
  }
  const jira = {
    ready: async () => [], statuses: async (keys: string) => { if (keys === 'invalid') throw new Error('falha simulada'); return { 'MB-1': 'READY' } },
    transition: async (key: string, status: string) => { if (!key || !status || key === 'INVALID') throw new Error('payload inválido'); return { status } },
  }
  const usage = { getUsage: async () => ({ fiveHour: null, sevenDay: null, fable: null }) }
  const coffee = { start: () => { coffeeActive = true }, stop: () => { coffeeActive = false }, active: () => coffeeActive }
  const routes = new Map<string, Route>([
    ['GET /api/workspace/settings', absoluteWorkspace], ['GET /api/workspace/detail', absoluteWorkspace],
    ['GET /api/chat', chatHistoryHttp(chat)],
    ['POST /api/chat/send', async (request) => { const result = await chatSendHttp(chat)(request); return { ...result, cancel: () => { cancelled += 1 } } }],
    ['GET /api/jira/ready', jiraReadyHttp(jira)], ['GET /api/jira/statuses', jiraStatusesHttp(jira)], ['POST /api/jira/transition', jiraTransitionHttp(jira)],
    ['GET /api/claude/usage', legacyJsonHandler(async (request) => {
      if (request.query.get('fail') === '1') throw new Error('falha simulada')
      return claudeUsageHttp(usage)(request)
    })],
    ['POST /api/coffee', legacyJsonHandler(async (request) => {
      if ((request.body as { fail?: boolean } | undefined)?.fail) throw new Error('falha simulada')
      return coffeeHttp(coffee)(request)
    })],
  ])
  return { routes, get cancelled() { return cancelled } }
}

async function request(transport: Transport, method: string, path: string, body?: unknown) {
  const response = await fetch(`${transport.base}${path}`, { method, headers: headers(transport, body === undefined ? {} : { 'Content-Type': 'application/json' }), body: body === undefined ? undefined : JSON.stringify(body) })
  return { status: response.status, headers: response.headers, json: await response.json() }
}
function headers(transport: Transport, value: Record<string, string>): Record<string, string> { return transport.name === 'node' ? { ...value, Authorization: `Bearer ${token}` } : value }
function notFound(response: ServerResponse): void { response.statusCode = 404; response.setHeader('Content-Type', 'application/json'); response.end(JSON.stringify({ error: 'Rota não encontrada' })) }
function listen(server: Server): Promise<void> { return new Promise((resolve, reject) => server.once('error', reject).listen(0, '127.0.0.1', () => { server.off('error', reject); resolve() })) }

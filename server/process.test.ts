...
import { repoDiff } from './workspace/service'
...
function fakeRunner(outputs: string[] = []): ProcessRunner & { calls: string[][] } {
  ...
  return {
    ...
    execFileSync(command, args) { calls.push([command, ...args]); return outputs.shift() ?? '' },
    execFile(command, args, _options, callback) { calls.push([command, ...args]); callback(null, outputs.shift() ?? '', '') },
  }
}

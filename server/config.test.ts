...
test('loads typed runtime configuration from a fixture environment without exposing secrets', () => {
  const config = loadMegaBrainConfig({
    ...
    env: {
      ...
      MEGA_BRAIN_CLAUDE_BIN: '/bin/claude',
      MEGA_BRAIN_GIT_BIN: '/bin/git',
      MEGA_BRAIN_CURSOR_BIN: '/bin/cursor',
      MEGA_BRAIN_TERMINAL_BIN: '/bin/terminal',
      MEGA_BRAIN_BROWSER_BIN: '/bin/browser',
      MEGA_BRAIN_POWERSHELL_BIN: '/bin/powershell',
    },
  })
  expect(config).toMatchObject({
    ...
    directories: {
      ...
      claudeProjects: '/tmp/projects',
      claudeCredentials: '/tmp/credentials.json',
    },
    executables: {
      claude: '/bin/claude',
      git: '/bin/git',
...
test('preserves web defaults for the current Vite environment', () => {
  ...
  expect(config).toMatchObject({
    mode: 'web',
    server: { host: '127.0.0.1', port: 0 },
    workspaceDir: './mega-brain-files/workspace',
    jira: {},
    directories: {
      claudeHome: '/tmp/home/.claude',
      claudeProjects: '/tmp/home/.claude/projects',
      claudeCredentials: '/tmp/home/.claude/.credentials.json',
    },
  })
})

test('rejects a configured public server host and accepts the dynamic port', () => {
  expect(() => loadMegaBrainConfig({ env: { MEGA_BRAIN_SERVER_HOST: '0.0.0.0' } })).toThrow(
    'bind público não é permitido',
  )
  expect(
    loadMegaBrainConfig({ env: { MEGA_BRAIN_SERVER_HOST: '127.0.0.1', MEGA_BRAIN_SERVER_PORT: '0' } }).server,

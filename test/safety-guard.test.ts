...
test('fails closed for a real workspace, Claude, shell, unsafe Git and Jira', () => {
  expect(() => assertSafeTestWorkspace('/home/user/real-workspace')).toThrow('TEST SAFETY')
  ...
  expect(() => assertSafeTestProcess('git', ['status'], { cwd: '/home/user/real-workspace' })).toThrow(
...
test('the installed setup guards block process, workspace and fetch boundaries', async () => {
  ...
  const workspace = createWorkspaceService({ workspaceDir: '/home/user/real-workspace', executables: {} })

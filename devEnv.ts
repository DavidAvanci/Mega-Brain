...
import type { DevEnvApp, DevEnvInfo } from './src/types'
import { nodeProcessRunner, type ProcessChild, type ProcessRunner } from './server/process'

const TAKEAT = join(homedir(), 'takeat')
const AGD = 'api-garcom-digital'
const CANONICAL_AGD = join(TAKEAT, 'backend', AGD)
const CLUBE = 'api-clube'
const CANONICAL_CLUBE = join(TAKEAT, 'clube', CLUBE)
const BACKEND_LIBS = new Set(['api-core', 'takeat-services'])
const BACKEND_PORT = Number(process.env.DEVENV_BACKEND_PORT || 3333)
const CLUBE_PORT = Number(process.env.DEVENV_CLUBE_PORT || 3334)
const LOCAL_API = `http://localhost:${BACKEND_PORT}`
const LOCAL_CLUBE_API = `http://localhost:${CLUBE_PORT}`
...
export const FRONTENDS: Record<string, FrontendConfig> = {
  'operation-takeat': { apiVar: 'VITE_OPERATION_API_URL', prodUrl: 'https://backend-pdv.takeat.app', script: 'dev', flavor: 'vite', port: 3000 },
  'garcom-restaurant-dashboard': { apiVar: 'REACT_APP_API_URL', prodUrl: 'https://backend.takeat.app', script: 'start', flavor: 'cra', port: 3000 },
  'manager-area': { apiVar: 'VITE_API_KEY', prodUrl: 'https://backend.takeat.app/', script: 'dev', flavor: 'vite', port: 5173 },
  'internal-dashboard': { apiVar: 'VITE_API_URL', prodUrl: 'https://backend.takeat.app', clubeApiVar: 'VITE_API_CLUB_URL', clubeProdUrl: 'https://backend.takeat.club', script: 'dev', flavor: 'vite', port: 5180 },
  'garcom-digital-client': { apiVar: 'VITE_API_URL', prodUrl: 'https://backend.takeat.app', script: 'start', flavor: 'vite', port: 5173 },
...
function git(dir: string, runner: ProcessRunner, ...args: string[]): string {
  try {
    return runner.execFileSync('git', ['-C', dir, ...args], { stdio: ['ignore', 'pipe', 'ignore'] })
      .toString()
      .trim()
  } catch {
    return ''
...
export function planDevEnv(cardPath: string, frontendChoice?: string, runner: ProcessRunner = nodeProcessRunner): DevEnvPlan | { needsFrontend: string[] } {
  ...
  return {
    ...
    fronts: frontRepos.map((front) => ({
      ...
      preferred: preferredPort(front.repo, frontRepos.map((f) => f.repo)),
    })),
    warnings,
  }
}
...
function processEnvironment(runner: ProcessRunner): NodeJS.ProcessEnv {
  ...
  if (wslLoginPath === undefined) {
    wslLoginPath = null
    try {
      const output = String(runner.execFileSync('bash', ['-lic', 'printf "MEGA_BRAIN_PATH=%s\\n" "$PATH"'], {
        encoding: 'utf8',
        stdio: ['ignore', 'pipe', 'ignore'],
      }))
      wslLoginPath = output.split('\n').find((line) => line.startsWith('MEGA_BRAIN_PATH='))?.slice('MEGA_BRAIN_PATH='.length).trim() || null
    } catch {}
  }
...
function step(run: Run, cwd: string, logFile: string, cmd: string, args: string[], runner: ProcessRunner): Promise<void> {
  checkAborted(run)
...
function detach(run: Run, cwd: string, logFile: string, cmd: string, args: string[], env: Record<string, string>, runner: ProcessRunner): ProcessChild {
  const fd = openSync(logFile, 'a')
  const child = runner.spawn(cmd, args, { cwd, detached: true, stdio: ['ignore', fd, fd], env: { ...processEnvironment(runner), ...env } })
  child.on('error', (error) => {
...
async function orchestrate(cardPath: string, run: Run, state: DevEnvInfo, plan: DevEnvPlan, runner: ProcessRunner): Promise<void> {
  ...
  try {
    if (plan.backend) {
      ...
      update()
      await step(run, cardPath, join(logDir, 'docker.log'), 'docker', ['start', 'takeat_db', 'takeat_redis'], runner).catch(() => {
      ...
      if (plan.backend.createWorktree) {
        ...
        update()
        await step(run, CANONICAL_AGD, join(logDir, 'worktree.log'), 'git', ['worktree', 'add', '--detach', plan.backend.dir, 'master'], runner)
      }
      ...
      run.child = detach(run, plan.backend.dir, logFile, ...argv(runScriptCommand(plan.backend.dir, 'dev')), { PORT: String(BACKEND_PORT) }, runner)
    ...
    if (plan.clube) {
      ...
      update()
      await step(run, cardPath, join(logDir, 'docker.log'), 'docker', ['start', 'database_clube_clientes', 'takeat_redis'], runner).catch(() => {
      ...
      run.child = detach(run, plan.clube.dir, logFile, ...argv(runScriptCommand(plan.clube.dir, 'dev')), { PORT: String(CLUBE_PORT) }, runner)
      app.pid = run.child.pid
      update()
    ...
    for (const front of plan.fronts) {
      ...
      run.child = detach(run, front.dir, logFile, ...argv(runScriptCommand(front.dir, front.config.script, extra)), env, runner)
      app.pid = run.child.pid
      update()
...
export function startDevEnv(cardPath: string, frontendChoice?: string, runner: ProcessRunner = nodeProcessRunner): { needsFrontend?: string[] } {
...
export function readDevEnv(cardPath: string): DevEnvInfo | null {
  ...
  const orchestrating =
    active.has(key) ||
    (state.ownerPid !== undefined && state.ownerPid !== process.pid && pidAlive(state.ownerPid))
  let changed = false
  for (const app of state.apps) {

...
export function createViteApiAdapter(options: ViteApiAdapterOptions) {
  ...
  return (req: IncomingMessage, res: ServerResponse, next: () => void): void => {
    ...
    const handler = options.routes?.[`${method} ${url.pathname}`] ?? options.routes?.[`* ${url.pathname}`] ?? options.fallback
    if (!handler) return next()
    const path = options.pathPrefix ? `${options.pathPrefix}${url.pathname === '/' ? '' : url.pathname}` : url.pathname
    readBody(req).then((body) => handler({ method, path, query: url.searchParams, headers: requestHeaders(req), body }))
      .then((result) => writeResult(res, result))
      .catch((error) => writeJson(res, legacyJsonError(error)))
  }
}

export function viteApiPlugin(name: string, mount: string, options: ViteApiAdapterOptions, onClose?: () => void): Plugin {

...
import { openDevEnv, openDevEnvAgent, startDevEnv, stopDevEnv } from './cards'
import { Tip } from './Tip'
import type { Card, DevEnvApp, DevEnvAppStatus } from './types'
...
export function DevEnvPanel({ card }: { card: Card }) {
  ...
  return (
    <div
      className="mt-2 flex flex-col gap-1 rounded-md border p-2"
    ...
    >
      ...
      {env.status === 'erro' && <p className="text-[11px] text-destructive">{env.error ?? 'Erro ao subir o ambiente'}</p>}

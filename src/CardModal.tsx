...
import { fetchDetail, moveCard, openFolder, openPrs, openTerminal, setCardFlow, type WorktreeRepoInfo } from './cards'
import { AgentBadge, activeAgents, agentName, PrChip, StageResetButton } from './CardView'
...
const ChatTab = lazy(() => import('./ChatTab').then((module) => ({ default: module.ChatTab })))
    ...
    <span className="inline-flex items-center gap-1 text-[10px] tabular-nums">
      <span
        className={cn(
          ...
          complete ? 'bg-emerald-500/15 text-emerald-700 dark:text-emerald-400' : 'bg-foreground/10 text-muted-foreground',
        )}
      >
        {counts.done}/{counts.total}
      </span>
      {counts.failed > 0 && (
        <span className="rounded-sm bg-destructive/10 px-1 py-px font-semibold text-destructive">
          {counts.failed}!
        </span>
      )}
      {counts.skipped > 0 && (
...
function CommitLink({ hash, shortHash }: { hash: string; shortHash: string }) {
  return <code title={hash} className="rounded bg-muted px-1.5 py-0.5 text-[11px]">{shortHash}</code>
}
      ...
      {repos.map((repo) => (
        <section key={repo.name} className="rounded-lg border p-3">
          <div className="flex min-w-0 items-center gap-2">
            ...
            {repo.dirty && <span className="rounded-sm bg-amber-500/15 px-1.5 py-0.5 text-[10px] font-medium text-amber-700 dark:text-amber-400">alterada</span>}
            ...
            <dl className="mt-3 grid grid-cols-[7rem_minmax(0,1fr)] gap-x-3 gap-y-2 text-xs">
              ...
              <dd className="min-w-0">
                {repo.base ? (
                  <span className="inline-flex flex-wrap items-center gap-1.5">
                    ...
                    {repo.base.inferred && <span className="text-[10px] text-muted-foreground" title="Worktree anterior ao registro de origem; base calculada pelo merge-base">inferida</span>}
                  </span>
              ...
              <dd className="min-w-0">
                <span className="inline-flex max-w-full flex-wrap items-center gap-1.5">
                  ...
                  <span className="truncate" title={repo.head.subject}>{repo.head.subject}</span>
                </span>
              </dd>
              <dt className="text-muted-foreground">Repositório</dt>
              <dd className="truncate font-mono text-[11px]" title={repo.remote ?? repo.repository}>{repo.remote ?? repo.repository}</dd>
              <dt className="text-muted-foreground">Worktree</dt>
              <dd className="truncate font-mono text-[11px]" title={repo.path}>{repo.path}</dd>
            </dl>
          )}
        </section>
      ))}
    ...
    <Dialog open onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="flex h-[92dvh] max-h-[880px] w-[calc(100%-2rem)] max-w-[1180px] flex-col gap-0 overflow-hidden p-0 shadow-2xl sm:h-[88dvh] sm:max-w-[1180px]">
        <div className="grid min-h-0 min-w-0 flex-1 grid-rows-[minmax(0,3fr)_minmax(260px,2fr)] lg:grid-cols-[minmax(0,1fr)_360px] lg:grid-rows-1">
          ...
          <Tabs
            defaultValue={initialTab && initialTab !== 'chat' ? initialTab : DEFAULT_TAB[card.status] ?? 'description'}
          ...
          >
            ...
            {card.description ? (
              ...
              <Placeholder>Sem descrição.</Placeholder>
            )}
            </TabsContent>
            {FILE_TABS.map((tab) => {
            const content = files?.[tab.file]
            ...
            <TabsContent value="diff" className="min-h-0 overflow-hidden">
              <Suspense fallback={<LoadingLines />}><DiffTab cardId={card.id} /></Suspense>
            </TabsContent>
          ...
          <aside className="flex min-h-0 min-w-0 flex-col border-t bg-muted/10 lg:border-t-0 lg:border-l" aria-label="Chat do card">
            <div className="shrink-0 border-b px-5 py-3">
              <h2 className="text-sm font-semibold">Chat</h2>
              <p className="text-[11px] text-muted-foreground">Converse com o agente desta task.</p>
            </div>
            <div className="min-h-0 flex-1">
              <Suspense fallback={<div className="px-5 py-4"><LoadingLines /></div>}><ChatTab cardId={card.id} /></Suspense>
            </div>
          </aside>
        </div>
      </DialogContent>

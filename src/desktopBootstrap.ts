import { ApiClient, configureApiClient } from './shared/api/api-client'
...
export type DesktopBootstrapFailure =
  | 'bridge-unavailable'
  | 'supervisor-failed'
  | 'invalid-session'
  | 'backend-starting'
...
): target is DesktopWindow & { __TAURI__: TauriBridge } {
...
function sessionFrom(value: unknown): BackendSession | undefined {
  ...
  if (typeof candidate.baseUrl !== 'string' || typeof candidate.token !== 'string' || !candidate.token.trim())
    return undefined
 ...
 */
export async function bootstrapDesktopApiClient(
  target: DesktopWindow | undefined = currentWindow(),
): Promise<ApiClient> {
...
export async function listDesktopWslDistributions(
  target: DesktopWindow | undefined = currentWindow(),
): Promise<string[]> {
  if (!isTauriDesktop(target)) throw new DesktopBootstrapError('bridge-unavailable')
...
export async function selectDesktopWslDistribution(
  distro: string,
  target: DesktopWindow | undefined = currentWindow(),
): Promise<void> {
  if (!isTauriDesktop(target)) throw new DesktopBootstrapError('bridge-unavailable')
...
export async function setDesktopWslWorkspaceDir(
  workspaceDir: string,
  target: DesktopWindow | undefined = currentWindow(),
): Promise<void> {
  if (!isTauriDesktop(target)) throw new DesktopBootstrapError('bridge-unavailable')

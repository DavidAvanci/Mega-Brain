...
function sameMetrics(left: ScrollMetrics, right: ScrollMetrics) {
  return left.clientWidth === right.clientWidth
...
export function BoardMinimap({ boardRef, cardsByStatus }: BoardMinimapProps) {
  ...
  const onPointerDown = (event: PointerEvent<HTMLDivElement>) => {
    ...
    const insideViewport = viewportBounds
      && event.clientX >= viewportBounds.left
      && event.clientX <= viewportBounds.right

    dragOffset.current = insideViewport && viewportBounds
      ? event.clientX - viewportBounds.left
      : (viewportBounds?.width ?? 0) / 2

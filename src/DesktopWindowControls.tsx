...
export function DesktopWindowControls({ onError }: { onError?: (message: string) => void }) {
  ...
  return (
    <div className="ml-1 flex self-stretch border-l">
      <button
        type="button"
        className="grid min-w-12 place-items-center text-muted-foreground hover:bg-muted hover:text-foreground"
        aria-label="Minimizar"
        onClick={() => run('minimize_main_window')}
      >
        <svg aria-hidden="true" viewBox="0 0 12 12" className="size-3.5 fill-none stroke-current" strokeWidth="1.25">
          <path d="M2 6h8" />
        </svg>
      </button>
      <button
      ...
      >
        <svg aria-hidden="true" viewBox="0 0 12 12" className="size-3.5 fill-none stroke-current" strokeWidth="1.1">
      ...
      <button
        type="button"
        className="grid min-w-12 place-items-center text-muted-foreground hover:bg-destructive hover:text-destructive-foreground"
        aria-label="Fechar"

...
import { abortChat, fetchChat, sendChat } from './cards'
import { Markdown } from './Markdown'
import type { ChatAgentSettings, ChatEntry } from './types'
...
function Entry({ entry }: { entry: ChatEntry }) {
  ...
  if (entry.role === 'user') {
    return (
      <p className="ml-auto max-w-[80%] rounded-lg bg-muted px-3 py-2 text-sm whitespace-pre-wrap">
        {entry.text}
      </p>
    )
  }
    ...
    <div className="flex h-full min-h-0 flex-col">
      ...
      <div className="flex items-end gap-2 border-t px-5 py-3">
        ...
        {streaming ? (
          ...
          <Button aria-label="Enviar mensagem" className="h-10 w-10 px-0" size="sm" onClick={send} disabled={!input.trim()}>

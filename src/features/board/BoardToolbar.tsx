...
export function BoardToolbar({
  ...
  const activeFilterCount =
    Number(preferences.attentionOnly) + Number(preferences.flow !== 'all') + Number(preferences.state !== 'all')
  ...
  return (
    <section className="border-b bg-card" aria-label="Controles do quadro">
      <div className="flex flex-wrap items-center gap-2 px-3 py-2.5 md:px-4">
        ...
        <div className="relative order-last w-full sm:order-none sm:min-w-56 sm:max-w-md sm:flex-1">
          ...
          {query ? (
            ...
            <kbd className="pointer-events-none absolute top-1/2 right-2 -translate-y-1/2 rounded border bg-background px-1.5 py-0.5 text-[10px] text-muted-foreground">
              /
            </kbd>
          )}
        </div>
        <div className="ml-auto flex items-center gap-1">
          ...
          <div className="flex rounded-lg border bg-background p-0.5" aria-label="Visualização">
            <Button
              variant={preferences.view === 'board' ? 'secondary' : 'ghost'}
              size="icon-xs"
              aria-label="Visualização em quadro"
              aria-pressed={preferences.view === 'board'}
              onClick={() => onPreferencesChange({ view: 'board' })}
            >
              <HugeiconsIcon icon={GridViewIcon} strokeWidth={2} />
            </Button>
            <Button
              variant={preferences.view === 'list' ? 'secondary' : 'ghost'}
              size="icon-xs"
              aria-label="Visualização em lista"
              aria-pressed={preferences.view === 'list'}
              onClick={() => onPreferencesChange({ view: 'list' })}
            >
      ...
      {filtersOpen && (
        <div
          className="flex flex-wrap items-center gap-2 border-t bg-muted/30 px-3 py-2 md:px-4"
        ...
        >
          <label className="flex items-center gap-1.5 text-xs text-muted-foreground">
            ...
            <AppSelect
              ...
              ariaLabel="Filtrar por fluxo"
              compact
              className="w-28"
              onValueChange={(flow) => onPreferencesChange({ flow })}
              options={[
          ...
          <label className="flex items-center gap-1.5 text-xs text-muted-foreground">
            ...
            <AppSelect
              ...
              compact
              className="w-36"
              onValueChange={(state) => onPreferencesChange({ state })}
              options={[
                { value: 'all', label: 'Todos' },
                { value: 'running', label: 'Agente rodando' },
                { value: 'waiting', label: 'Aguardando' },
                { value: 'error', label: 'Com erro' },
                { value: 'pr', label: 'Com PR' },
              ]}
            />
          </label>
          <FilterToggle
            pressed={preferences.attentionOnly}
            onClick={() => onPreferencesChange({ attentionOnly: !preferences.attentionOnly })}
          >
            Precisa de atenção
          </FilterToggle>
          {(preferences.attentionOnly || preferences.flow !== 'all' || preferences.state !== 'all') && (
            <Button
              ...
              onClick={() => onPreferencesChange({ attentionOnly: false, flow: 'all', state: 'all' })}
            >
              Limpar opções
            </Button>
          )}

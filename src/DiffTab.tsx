...
const LOCK_FILE = /(^|\/)(package-lock\.json|yarn\.lock|pnpm-lock\.yaml|bun\.lockb?|composer\.lock|Cargo\.lock|Gemfile\.lock|poetry\.lock)$/
...
const collapseState = new Map<string, { collapsed: ReadonlySet<string>; collapsedRepos: ReadonlySet<string>; seen: Set<string> }>()
...
function FileHeader({
  ...
  return (
    ...
    >
      ...
      {badge && <span className={cn('shrink-0 rounded-sm px-1 py-px text-[10px]', badge.className)}>{badge.label}</span>}
...
export function DiffTab({ cardId }: { cardId: string }) {
  ...
  useEffect(() => {
    ...
    fetchDiff(cardId)
      .then((data) => {
        ...
        const state = collapseState.get(cardId) ?? { collapsed: new Set<string>(), collapsedRepos: new Set<string>(), seen: new Set<string>() }
  ...
  const rows = useMemo<Row[]>(() => {
    ...
    return list.flatMap((repo) => [
      ...
      ...(collapsedRepos.has(repo.name) ? [] : repo.files.map((file) => ({
        type: 'file' as const,
  ...
  return (
    <div className="flex h-full min-h-0 flex-col">
      ...
      <div ref={scroll} onScroll={(event) => setScrollTop(event.currentTarget.scrollTop)} className="min-h-0 flex-1 overflow-y-auto px-5 py-4">
        {empty ? (
        <p className={cn('py-10 text-center text-xs', error ? 'text-destructive' : 'text-muted-foreground')}>{empty}</p>
      ) : repos === null ? (
        ...
        <>
          {showStickyHeaders && (
            <div className="sticky -top-4 z-100 h-0 -mx-5 overflow-visible" style={{ zIndex: 100 }}>
              ...
              >
                <HugeiconsIcon icon={activeRepo && !collapsedRepos.has(activeRepo) ? ArrowDown01Icon : ArrowRight01Icon} strokeWidth={2} className="size-3" />
                Projeto: {activeRepo ?? '—'}
              </button>
              {activeFile && (
                ...
                >
                  {row.type === 'repo' ? (
                    <>
                      ...
                      >
                        <HugeiconsIcon icon={collapsedRepos.has(row.name) ? ArrowRight01Icon : ArrowDown01Icon} strokeWidth={2} className="size-3" />
                        {row.name}
                      </button>
